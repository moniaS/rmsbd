...
CREATE TABLE Klienci(
	ID NUMBER(10) NOT NULL PRIMARY KEY,
  Nazwa_uzytkownika VARCHAR2(50 CHAR)
);
...
CREATE TABLE Opinie (
  ...
  ID_klienta NUMBER(10) NOT NULL,
	CONSTRAINT O1 FOREIGN KEY (ID_restauracji) REFERENCES Restauracje (ID),
  CONSTRAINT O2 FOREIGN KEY (ID_klienta) REFERENCES Klienci (ID)
);

CREATE SEQUENCE Opinie_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER Opinie_seq_tr
 BEFORE INSERT ON Opinie FOR EACH ROW
 WHEN (NEW.ID IS NULL)
BEGIN
 SELECT Opinie_seq.NEXTVAL INTO :NEW.ID FROM DUAL;
END;
